        ...
        <div>
          ...
          <ul>  {/* 21. Create a list of users returned from the API call */}
              {users.map((user) => {   // 22. Map over the users state variable and return the user's name, email, and phone number to the UI 
                const { login, name, picture } = user; // 23. Destructure the user object to get the login, name, and picture properties  
                return (   
                    <li key={login.uuid}>  {/* 24. Create a list item for each user returned from the API call */}
                      <img src={picture.medium} alt={name.first} /> {/* 25. Display the user's picture */}
                      <div>
                        {name.first} {name.last} {/* 26. Display the user's first and last name */}
                    </div>
                    <div>
                        {login.username} {login.password}  {/* 27. Display the user's username and password */}
                    </div>
                    <div>
                        {login.email} {login.phone}  {/* 28. Display the user's email and phone number */}
                    </div>
                    </li>  
                );
              })}
